...
export const AdminMenuScreen: React.FC<AdminMenuScreenProps & { onLogout?: () => void }> = ({
  onNavigate,
  onLogout,
}) => {
  const menuItems = [
  ...
  return (
    <div className="p-4 space-y-6 animate-page pb-24">
        ...
        {menuItems.map((item) => (
          ...
          >
            <Card className="flex items-center p-4 hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors border border-gray-100 dark:border-gray-700 shadow-sm">
              <div
                className={`w-10 h-10 rounded-lg ${item.bg} flex items-center justify-center ${item.color} mr-4`}
              >
      ...
      <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
        <button onClick={handleLogout} className="w-full text-left group focus:outline-none">
