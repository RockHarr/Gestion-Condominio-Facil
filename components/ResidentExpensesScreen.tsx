...
import { Expense, ExpenseStatus, Page } from '../types';
...
export const ResidentExpensesScreen: React.FC<ResidentExpensesScreenProps> = ({
  expenses,
  showToast,
  onNavigate,
}) => {
  const approvedExpenses = useMemo(
    () => expenses.filter((e) => e.status === ExpenseStatus.APROBADO),
    [expenses],
  );
  ...
  return (
    <div className="animate-page pb-24">
      <Header title="Gastos Comunidad" onBack={() => onNavigate('home')} />
                  ...
                  <div className="flex justify-between items-center gap-4">
                    <div>
                      ...
                      <p className="text-sm text-gray-500 dark:text-gray-400">
                        {expense.proveedor} - {new Date(expense.fecha).toLocaleDateString('es-CL')}
                      </p>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <p className="font-bold">{formatCurrency(expense.monto)}</p>
                      <button
                        onClick={() => showToast('Simulando vista de boleta...', 'info')}
