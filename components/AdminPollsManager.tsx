...
export const AdminPollsManager: React.FC = () => {
  ...
  const handleCloseEarly = async () => {
    ...
    if (
      !window.confirm(
        '¿Está seguro de cerrar esta encuesta anticipadamente? Esta acción es irreversible.',
      )
    )
      return;
    ...
    <div className="p-6 max-w-6xl mx-auto">
      ...
      <div className="grid gap-6">
        {polls.map((poll) => {
          const isActive =
            !poll.closed_at &&
            new Date() >= new Date(poll.start_at) &&
            new Date() < new Date(poll.end_at);
              ...
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-sm text-gray-600 dark:text-gray-300">
                    ...
                    {new Date(poll.end_at).toLocaleDateString()}
                  </p>
                  {poll.closed_at && (
                    <p className="text-sm text-red-600 mt-1">
                      <strong>Cierre Anticipado:</strong> {poll.close_reason} (
                      {new Date(poll.closed_at).toLocaleString()})
                    </p>
                  )}
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewPoll({ ...newPoll, showResultsWhen: e.target.value as any })
                    }
                  >
                ...
                {newPoll.options.map((opt, idx) => (
                  <div key={idx} className="flex gap-2 mb-2">
                    ...
                    {newPoll.options.length > 2 && (
                      <Button variant="secondary" onClick={() => removeOption(idx)}>
                        X
                      </Button>
                    )}
                  </div>
                ))}
                <Button variant="secondary" onClick={addOption} className="w-full mt-2">
                  + Agregar Opción
                </Button>
              </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md">
            ...
            <div className="flex justify-end gap-2">
              ...
              <Button variant="danger" onClick={handleCloseEarly}>
                Confirmar Cierre
              </Button>
            </div>
          </div>
