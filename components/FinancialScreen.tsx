        ...
        <Card key={statement.id}>
          ...
          <div className="mt-2 text-sm space-y-1">
            <div className="flex justify-between">
              ...
              <span>{formatCurrency(statement.ingresos)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-red-600 dark:text-red-400">Egresos:</span>
              <span>{formatCurrency(statement.egresos)}</span>
            </div>
            <div className="flex justify-between font-semibold">
              <span className="text-gray-800 dark:text-gray-200">Saldo:</span>
              <span>{formatCurrency(statement.saldo)}</span>
            </div>
          </div>
          <Button
            onClick={() => showToast('Simulando descarga de informe...', 'info')}
            variant="secondary"
            className="mt-4 !py-2 !text-sm"
          >
            <div className="flex items-center justify-center">
              <Icons name="download" className="w-4 h-4 mr-2" /> Descargar Informe
            </div>
          </Button>
        </Card>
      ))}
      ...
      <Card>
        ...
        <div className="my-6 text-center">
          ...
          <p className="text-4xl font-bold text-gray-900 dark:text-white">
            {formatCurrency(fund.montoActual)}
          </p>
        </div>
        <div>
          ...
          <p className="text-center text-sm font-semibold mt-2 text-gray-700 dark:text-gray-300">
