...
export const AdminTabBar: React.FC<AdminTabBarProps> = ({
  ...
  const getActiveTab = () => {
    // Direct match
    if (
      ['admin-dashboard', 'admin-payment-entry', 'admin-requests', 'admin-menu'].includes(
        currentPage,
      )
    )
      return currentPage;
    ...
    // Requests sub-pages
    if (['admin-tickets', 'admin-reservations', 'admin-ticket-detail'].includes(currentPage))
  ...
  return (
    <aside className="hidden md:flex md:flex-col md:w-72 bg-white dark:bg-gray-900 border-r border-gray-200 dark:border-gray-800 shadow-xl z-30">
      ...
      <nav className="flex-1 px-4 py-6 space-y-1 overflow-y-auto">
        ...
        {navItems.map((item) => {
          const isActive =
            currentPage === item.page || (currentPage as string).startsWith(item.page);
          return (
            <button
              key={item.page}
              onClick={() => onNavigate(item.page as Page)}
