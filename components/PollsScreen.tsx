  ...
  const loadOptionsAndResults = async () => {
    ...
    for (const poll of polls) {
      ...
      try {
        ...
        console.error(`Error loading options for poll ${poll.id}`, e);
      }

      // Load Results if visible
      const isClosed = poll.closed_at || new Date() >= new Date(poll.end_at);
      const canViewResults =
        poll.show_results_when === 'LIVE' || (poll.show_results_when === 'CLOSED' && isClosed);
      ...
      if (canViewResults) {
        try {
          ...
          console.error(`Error loading results for poll ${poll.id}`, e);
        }
      }
    }
    setPollOptions(opts);
                    ...
                    {pollResults ? (
                      <div className="space-y-3">
                          ...
                          <div key={res.option_id}>
                            <div className="flex justify-between text-sm mb-1">
                              <span className="font-medium text-gray-700 dark:text-gray-300">
                                {res.option_text}
                                {myVotes[poll.id] === res.option_id && (
                                  <span className="ml-2 text-blue-500">(Tu voto)</span>
                                )}
                              </span>
                        ...
                        <p className="text-xs text-center text-gray-500 mt-2">
                          Total votos:{' '}
                          {pollResults.reduce((acc, curr) => acc + Number(curr.votes_count), 0)}
                        </p>
                      </div>
                    ) : (
