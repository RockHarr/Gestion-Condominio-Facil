...
export const Button: React.FC<{ onClick?: (e: React.MouseEvent<HTMLButtonElement>) => void, children: React.ReactNode, variant?: 'primary' | 'secondary' | 'danger' | 'ghost', className?: string, type?: 'button' | 'submit' | 'reset', disabled?: boolean, 'data-testid'?: string, 'aria-label'?: string }> = ({ onClick, children, variant = 'primary', className = '', type = 'button', disabled = false, ...rest }) => {
    ...
    return <button type={type as "button" | "submit" | "reset"} onClick={onClick} className={`${baseClasses} ${variantClasses[variant as keyof typeof variantClasses]} ${className}`} disabled={disabled} {...rest}>{children}</button>;
};
