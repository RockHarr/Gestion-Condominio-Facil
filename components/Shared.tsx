...
export const Card: React.FC<{
  ...
  <div
    className={`bg-white dark:bg-gray-800 rounded-xl shadow-md p-4 sm:p-6 ${className}`}
    onClick={onClick}
  >
    {children}
  </div>
...
export const Button: React.FC<{
  ...
  const baseClasses =
    'w-full text-center px-4 py-3 rounded-lg font-semibold text-base focus:outline-none focus:ring-2 focus:ring-offset-2 dark:focus:ring-offset-gray-900 transition-transform transform active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed';
...
export const Header: React.FC<{
  ...
  onLogout?: () => void;
  children?: React.ReactNode;
}> = ({ title, onBack, onLogout, children }) => (
  <header className="sticky top-0 z-10 bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-700 p-4 flex items-center justify-between">
    <div className="flex items-center min-w-0">
      {onBack && (
        <button
          onClick={onBack}
          className="mr-3 p-2 -ml-2 rounded-full text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-500 flex-shrink-0"
          aria-label="Volver"
        >
...
export const EmptyState: React.FC<{
  ...
  <div
    className={`text-center py-12 px-4 bg-gray-50 dark:bg-gray-800/30 rounded-2xl border border-dashed border-gray-200 dark:border-gray-700 ${className}`}
  >
