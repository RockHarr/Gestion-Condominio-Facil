                ...
                <div className="space-y-4">
                    {itemsToPay.length > 0 ? itemsToPay.map((item, index) => (
                        ...
                        <div className="text-center py-8">
                            ...
                            <p className="text-gray-600 dark:text-gray-400 font-medium">No tienes deudas pendientes.</p>
                        </div>
                    )}
                    ...
                    {itemsToPay.length > 0 && (
                        <>
                            ...
                            <div className="flex justify-between items-end">
                                ...
                                <span className="text-3xl font-bold text-blue-600 dark:text-blue-400">{formatCurrency(totalAmount)}</span>
                            </div>
                        </>
                    )}
                </div>
...
export const PaymentMethodScreen: React.FC<{ onNavigate: (page: Page, params?: any) => void, params: { totalAmount: number, itemsToPay: any[] } }> = ({ onNavigate, params }) => {
    ...
    return (
        <div className="p-4 space-y-6 animate-page">
            <div className="flex items-center justify-center space-x-2 mb-6">
                <div className="w-8 h-8 rounded-full bg-green-500 text-white flex items-center justify-center font-bold text-sm"><Icons name="check" className="w-4 h-4" /></div>
            ...
            <Card>
                ...
                <div className="space-y-3">
                    {[
                        ...
                        <label key={method.id} className={`flex items-center p-4 border rounded-xl cursor-pointer transition-all ${selectedMethod === method.id ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20 ring-1 ring-blue-500' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300'}`}>
                            <input
                                type="radio"
                                name="paymentMethod"
                                value={method.id}
                            ...
                            <div className="ml-4 flex items-center">
                                <div className={`p-2 rounded-lg ${selectedMethod === method.id ? 'bg-blue-100 dark:bg-blue-800 text-blue-600 dark:text-blue-200' : 'bg-gray-100 dark:bg-gray-800 text-gray-500'}`}>
                                    <Icons name={method.icon as any} className="w-6 h-6" />
                                </div>
                                <span className="ml-3 font-semibold text-gray-900 dark:text-white">{method.name}</span>
                            </div>
                        </label>
                    ))}
                </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 p-4 bg-white dark:bg-gray-900 border-t border-gray-200 dark:border-gray-800">
                <div className="max-w-md mx-auto flex justify-between items-center mb-4">
                    ...
                    <span className="text-xl font-bold text-gray-900 dark:text-white">{formatCurrency(params.totalAmount)}</span>
                </div>
                <Button onClick={() => onNavigate('payment-confirm', params)} className="shadow-lg shadow-blue-500/30">
    ...
    return (
        <div className="p-4 space-y-6 animate-page flex flex-col h-full justify-center">
            ...
            <Card className="text-center py-8">
                ...
                <p className="text-gray-500 dark:text-gray-400 mt-2">Estás a un paso de completar tu pago.</p>
                ...
                <div className="mt-8 bg-gray-50 dark:bg-gray-700/30 p-4 rounded-xl text-left">
                    ...
                    <div className="flex justify-between mb-2">
                        ...
                        <span className="font-semibold text-gray-900 dark:text-white">{formatCurrency(params.totalAmount)}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-gray-600 dark:text-gray-400">Orden de Compra</span>
                        <span className="font-mono text-gray-900 dark:text-white">#{Math.floor(Math.random() * 1000000)}</span>
                    </div>
                </div>
            </Card>

            <div className="space-y-3 pt-4">
                <Button onClick={handleConfirm} disabled={isLoading} className="shadow-lg shadow-blue-500/30">
                    {isLoading ? (
        ...
        <div className="p-4 space-y-6 animate-page">
            <Card className="relative overflow-hidden !p-0 border-0 shadow-2xl">
                ...
                <div className="bg-green-600 p-6 text-center text-white relative">
                    <div className="absolute top-0 left-0 w-full h-full bg-white opacity-10" style={{ backgroundImage: 'radial-gradient(circle, #fff 2px, transparent 2.5px)', backgroundSize: '20px 20px' }}></div>
                ...
                <div className="p-6 bg-white dark:bg-gray-800 relative">
                    {/* Jagged edge effect */}
                    <div className="absolute top-0 left-0 w-full h-4 -mt-2 bg-white dark:bg-gray-800" style={{ clipPath: 'polygon(0% 0%, 5% 100%, 10% 0%, 15% 100%, 20% 0%, 25% 100%, 30% 0%, 35% 100%, 40% 0%, 45% 100%, 50% 0%, 55% 100%, 60% 0%, 65% 100%, 70% 0%, 75% 100%, 80% 0%, 85% 100%, 90% 0%, 95% 100%, 100% 0%)' }}></div>

                    <div className="space-y-4 pt-2">
                        <div className="text-center mb-6">
                            <p className="text-gray-500 text-sm uppercase tracking-wider">Monto Pagado</p>
                            <p className="text-4xl font-bold text-gray-900 dark:text-white">{formatCurrency(params.totalAmount)}</p>
                        </div>

                        <div className="border-t border-dashed border-gray-200 dark:border-gray-700 pt-4 space-y-2 text-sm">
                            <div className="flex justify-between">
                                <span className="text-gray-500">Fecha</span>
                                <span className="font-medium text-gray-900 dark:text-white">{new Date().toLocaleDateString('es-CL', { hour: '2-digit', minute: '2-digit' })}</span>
                            </div>
                            <div className="flex justify-between">
                                <span className="text-gray-500">ID Transacción</span>
                                <span className="font-mono font-medium text-gray-900 dark:text-white">{Math.random().toString(36).substring(2, 10).toUpperCase()}</span>
                            </div>
                            <div className="flex justify-between">
                ...
                <div className="bg-gray-50 dark:bg-gray-900/50 p-4 border-t border-gray-100 dark:border-gray-700 flex gap-3">
                    <Button onClick={() => showToast('Descargando PDF...', 'info')} variant="secondary" className="!py-2 !text-sm">
                        <div className="flex items-center justify-center gap-2">
                            <Icons name="download" className="w-4 h-4" /> PDF
                        </div>
                    </Button>
                    <Button onClick={() => showToast('Enlace copiado', 'success')} variant="secondary" className="!py-2 !text-sm">
                        <div className="flex items-center justify-center gap-2">
