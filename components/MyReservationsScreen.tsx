                                                ...
                                                ${
                                                  reservation.status === ReservationStatus.CONFIRMED
                                                    ...
                                                    : reservation.status ===
                                                      ...
                                                      : reservation.status ===
                                                          ReservationStatus.APPROVED_PENDING_PAYMENT
                                                        ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-400'
                                                        : 'bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300'
                                                }`}
                      >
                        {reservation.status === ReservationStatus.CONFIRMED && (
                          <Icons name="check-badge" className="w-3.5 h-3.5" />
                        )}
                        ...
                        {reservation.status === ReservationStatus.APPROVED_PENDING_PAYMENT && (
                          <Icons name="currency-dollar" className="w-3.5 h-3.5" />
                        )}
                ...
                {charges && showPayButton && (
                  <div className="mt-4 border-t border-gray-200 dark:border-gray-700 pt-3 animate-in fade-in">
                        ...
                        >
                          <div>
                            <p className="text-sm font-medium text-gray-800 dark:text-gray-200">
                              {charge.type === 'RESERVATION_FEE'
                                ? 'Tarifa de Uso'
                                : charge.type === 'RESERVATION_DEPOSIT'
                                  ? 'Garantía'
                                  : charge.type}
                            </p>
                    ...
                    <p className="text-xs text-gray-500 mt-2 italic">
                      * La reserva se confirmará automáticamente cuando todos los cargos estén
                      pagados.
                    </p>
                  </div>
                )}
