        ...
        <div className="p-6 max-w-5xl mx-auto space-y-6 animate-page pb-24">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 mt-1">Administra las solicitudes de los residentes</p>
                </div>
            ...
            <div className="flex space-x-1 bg-gray-100 dark:bg-gray-800 p-1 rounded-xl overflow-x-auto">
                {(['TODOS', ...Object.values(TicketStatus)] as const).map(status => (
                    <button
                        ...
                        className={`px-4 py-2 text-sm font-medium rounded-lg whitespace-nowrap transition-all duration-200 flex-1 ${filter === status
            ...
            <div className="space-y-4">
                {filteredTickets.length > 0 ? (
                    filteredTickets.map(ticket => (
                        <button key={ticket.id} onClick={() => onNavigate('admin-ticket-detail', { id: ticket.id })} className="w-full text-left group">
                                ...
                                <div className="flex flex-col sm:flex-row justify-between items-start gap-4">
                                    <div className="flex-1">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-xs text-gray-400 dark:text-gray-500">• {new Date(ticket.fecha).toLocaleDateString('es-CL')}</span>
                                        </div>
                                        ...
                                        <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-800/50 px-3 py-1.5 rounded-lg w-fit">
                                            ...
                                            <span className="font-medium text-gray-700 dark:text-gray-300">{ticket.user?.nombre}</span>
...
export const AdminTicketDetailScreen: React.FC<AdminTicketDetailScreenProps> = ({ ticket, onUpdateStatus }) => {
            ...
            <Card className="bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900/30">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-end">
                    <div className="w-full">
                        <label htmlFor="status-select" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Nuevo Estado</label>
                        <select
                            id="status-select"
                            value={newStatus}
