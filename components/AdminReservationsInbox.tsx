...
export const AdminReservationsInbox: React.FC<AdminReservationsInboxProps> = ({
  reservations,
  onNavigate,
  onRefreshData,
}) => {
  ...
  const [selectedReservationForIncident, setSelectedReservationForIncident] =
    useState<Reservation | null>(null);
  const [selectedReservationForDeposit, setSelectedReservationForDeposit] =
    useState<Reservation | null>(null);
      ...
      if (activeTab === 'pending') {
        ...
        return (
          (r.status === ReservationStatus.CONFIRMED ||
            r.status === ReservationStatus.APPROVED_PENDING_PAYMENT) &&
          new Date(r.startAt) > new Date()
        );
  ...
  const counts = {
    ...
    history: reservations.filter(
      (r) =>
        ...
        (r.status === ReservationStatus.CONFIRMED && new Date(r.startAt) <= new Date()),
    ).length,
      ...
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Administra las solicitudes y uso de espacios comunes
          </p>
        </div>
      ...
      {selectedReservationForPayment && (
        <ReservationPaymentModal
          ...
          onSuccess={async (paymentData) => {
            try {
              await dataService.confirmReservationPayment(
      ...
      <div className="space-y-4">
        {filteredReservations.length === 0 ? (
          ...
          filteredReservations.map((reservation) => (
            <Card
              key={reservation.id}
              className={`p-4 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 ${reservation.isSystem ? 'border-l-4 border-orange-500' : ''}`}
            >
              <div className="flex-1">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  {reservation.isSystem && (
                    <span className="text-xs text-orange-600 font-bold">SISTEMA</span>
                  )}
                </div>
                ...
                {reservation.user ? (
                  ...
                  reservation.userId && (
                    <p className="text-xs text-gray-500 mt-1">Usuario ID: {reservation.userId}</p>
                  )
                )}
              </div>
                  ...
                  <>
                    {(reservation.status === ReservationStatus.COMPLETED ||
                      ...
                      <>
                        <Button
                          variant="secondary"
                          onClick={() => setSelectedReservationForDeposit(reservation)}
                        >
                          Gestionar Garantía
                        </Button>
                        <Button
                          variant="danger"
                          onClick={() => setSelectedReservationForIncident(reservation)}
                        >
                          Reportar Incidente
                        </Button>
                      </>
