    ...
    return (
        <div className="space-y-6 animate-page pb-24">
            ...
            <div className="px-4 pt-6 pb-2">
                <h1 className="text-3xl font-bold text-gray-900 dark:text-white tracking-tight">Hola, {user.nombre.split(' ')[0]}</h1>
            ...
            <div className="px-4 space-y-6">
                ...
                {/* Quick Actions */}
                <div>
                    <h2 className="text-lg font-bold text-gray-800 dark:text-white mb-3 px-1">Accesos Rápidos</h2>
                    <div className="grid grid-cols-2 gap-4">
                        <button onClick={() => onNavigate('reservations')} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col items-center justify-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                            ...
                            <span className="font-semibold text-gray-700 dark:text-gray-200 text-sm">Reservar</span>
                        </button>
                        <button onClick={() => onNavigate('ticket-create')} className="bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 flex flex-col items-center justify-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors group">
                            ...
                            <span className="font-semibold text-gray-700 dark:text-gray-200 text-sm">Nuevo Ticket</span>
                        </button>
                    </div>
                </div>
                ...
                <Card className="!p-0 overflow-hidden border border-gray-100 dark:border-gray-700">
                    <div className="p-5 border-b border-gray-100 dark:border-gray-700 bg-gray-50/50 dark:bg-gray-800/50">
                        <div className="flex justify-between items-center mb-1">
                            ...
                            <span className="text-xs font-medium text-gray-500 bg-gray-200 dark:bg-gray-700 px-2 py-1 rounded-md">{formatPeriod(currentMonthPeriod)}</span>
                        </div>
                        <p className="text-3xl font-bold text-gray-900 dark:text-white mt-2">{formatCurrency(totalApprovedAmount)}</p>
                                ...
                                {approvedExpensesThisMonth.slice(0, 3).map(expense => (
                                    <li key={expense.id} className="flex justify-between items-center">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p className="font-medium text-sm text-gray-900 dark:text-white line-clamp-1">{expense.descripcion}</p>
                                                <p className="text-xs text-gray-500 dark:text-gray-400">{expense.categoria}</p>
                                            </div>
                                        </div>
                                        <span className="font-semibold text-sm text-gray-900 dark:text-white">{formatCurrency(expense.monto)}</span>
                                    </li>
                                ))}
                            </ul>
                        ) : (
