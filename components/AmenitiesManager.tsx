...
export const AmenitiesManager: React.FC<AmenitiesManagerProps> = ({ onNavigate }) => {
    ...
    const handleDelete = async (id: number) => {
        if (!window.confirm('¿Estás seguro de eliminar este espacio? Se borrarán todas las reservas asociadas.')) return;
        ...
        <div className="p-6 max-w-7xl mx-auto space-y-8 animate-page pb-24">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 dark:text-gray-400 ml-7">Gestiona los quinchos, salas y áreas reservables.</p>
                </div>
            ...
            {loading ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {amenities.map((amenity) => (
                        <Card key={amenity.id} className="group hover:shadow-lg transition-all duration-200 border border-gray-100 dark:border-gray-700">
                            ...
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-1">{amenity.name}</h3>
                    ...
                    {amenities.length === 0 && (
                        <div className="col-span-full text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-2xl border-2 border-dashed border-gray-200 dark:border-gray-700">
                            ...
                            <h3 className="text-lg font-medium text-gray-900 dark:text-white">No hay espacios creados</h3>
                            <p className="text-gray-500 dark:text-gray-400 mt-1">Comienza creando el primer espacio común.</p>
                        </div>
                    )}
                </div>
            )}
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
                    <Card className="w-full max-w-lg shadow-2xl animate-in zoom-in-95 duration-200">
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">URL Foto (Opcional)</label>
                                <input
                                    value={photoUrl}
