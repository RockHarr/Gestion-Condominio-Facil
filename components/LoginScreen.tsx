...
export const LoginScreen: React.FC<LoginScreenProps> = () => {
  ...
  const checkConnection = async () => {
    try {
      ...
      setConnectionError(
        `No se puede conectar con el servidor: ${getErrorMessage(err)}. Verifique su conexión y la configuración en .env.local`,
      );
    }
  };
    ...
    try {
      ...
      if (error) {
        ...
        if (
          ...
          setConnectionError(
            'No se pudo conectar con el servidor. Verifique su internet y credenciales.',
          );
        }
      } else {
        if (usePassword) {
      ...
      setMessage({ text: 'Error crítico: ' + getErrorMessage(err), type: 'error' });
    } finally {
      setLoading(false);
    }
  };
        ...
        <form onSubmit={handleLogin} className="mt-8 space-y-6">
              ...
              <div className="animate-in fade-in slide-in-from-top-2 duration-200">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 rounded-full"
          ...
          {connectionError && (
            <div className="p-3 bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-400 rounded text-sm border border-yellow-200 dark:border-yellow-700">
              ...
              <div className="mt-2 text-xs">
                Sugerencia: Revise que las variables VITE_SUPABASE_URL y ANNON_KEY sean correctas en
                .env.local
              </div>
            </div>
          )}
          ...
          <div className="space-y-3">
            ...
            <button
              ...
              className="w-full text-sm text-blue-600 dark:text-blue-400 hover:underline focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 focus-visible:ring-offset-2 rounded"
            >
