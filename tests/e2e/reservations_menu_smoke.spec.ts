...
test('reservations_menu_smoke', async ({ page }) => {
    ...
    if (await page.getByText('Iniciar Sesión').isVisible()) {
        await page.fill('input[type="email"]', process.env.TEST_ADMIN_EMAIL || 'admin@condominio.com');
        await page.click('button:has-text("Usar contraseña")');
        await page.fill('input[type="password"]', process.env.TEST_ADMIN_PASSWORD || 'admin123'); // Assuming test creds
