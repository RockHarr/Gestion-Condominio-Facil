...
test.describe('Resident — Reservations Flow', () => {
  ...
  test('should allow a resident to create and cancel a reservation', async ({ page }) => {
    ...
    if (
      await modal.getByText('No hay tipos de reserva disponibles para este espacio.').isVisible()
    ) {
    ...
    try {
      await expect(page.getByText('Solicitud de reserva enviada exitosamente.')).toBeVisible({
        timeout: 5000,
      });
    } catch (e) {
