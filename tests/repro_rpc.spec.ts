...
test.describe('Repro RPC', () => {
    test.skip(!checkTestEnv(), 'Skipping test: Missing environment variables (TEST_*)');

    // Use dummy values if env is missing to prevent createClient from throwing during discovery
    // The test is skipped anyway, so this client won't be used
