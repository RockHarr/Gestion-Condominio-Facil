...
- Name: e2e/admin_reservations.spec.ts >> Admin — Reservations Management >> should allow admin to approve a pending reservation
- Location: tests/e2e/admin_reservations.spec.ts:119:5
...
Locator: getByRole('heading', { name: 'Inicio', exact: true }).first()
Expected: visible
Timeout: 15000ms
...
Call log:
  ...
  - waiting for getByRole('heading', { name: 'Inicio', exact: true }).first()
...
```ts
  ...
  7   | const RESIDENT_PASSWORD = '180381';
  8   | const ADMIN_EMAIL = 'rockwell.harrison@gmail.com';
  9   | const ADMIN_PASSWORD = '270386';
  10  | // ==========================================
  11  |
  ...
  24  |
  25  |         // Wait for login
> 26  |         await expect(page.getByRole('heading', { name: 'Inicio', exact: true }).first()).toBeVisible({ timeout: 15000 });
      |                                                                                          ^ Error: expect(locator).toBeVisible() failed
  ...
  43  |
  44  |                 await page.click('button:has-text("Espacios")');
  45  |                 await page.locator('button:has-text("Reservar")').first().click();
  46  |             } else {
  47  |                 // Initial navigation
  48  |                 await page.click('button:has-text("Espacios")');
