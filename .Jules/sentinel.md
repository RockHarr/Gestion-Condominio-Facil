...
## 2026-06-06 - [Hardcoded Credentials in Test Scripts]
**Vulnerability:** E2E test files (`tests/e2e/*.spec.ts`) and utility scripts (`scripts/*`) contained hardcoded Supabase URLs (`https://tqshoddiisfgfjqlkntv.supabase.co`), API keys, and resident/admin passwords. This exposed potentially sensitive environment details and credentials within the repository.
**Learning:** Even though test files might only connect to test environments or use non-production accounts, hardcoding any credentials violates secure coding principles and risks accidental exposure of real keys if environments are reused or copied.
**Prevention:** Always use environment variables (e.g., `process.env.VITE_SUPABASE_URL`, `process.env.TEST_RESIDENT_PASSWORD`) with safe default fallback values (like `http://127.0.0.1:54321` or `dummy_key`) for tests and utility scripts. Ensure CI workflows provide these dummy values via the `env` block.
