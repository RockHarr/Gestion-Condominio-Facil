...
## 2026-05-19 - Added Content-Security-Policy Header
**Vulnerability:** The application was missing critical security headers like `Content-Security-Policy` (CSP), `X-Frame-Options`, `X-Content-Type-Options`, and `X-XSS-Protection`. This leaves the frontend exposed to Cross-Site Scripting (XSS), Clickjacking, and MIME-sniffing attacks.
**Learning:** Modern frontend deployments, especially static sites on Vercel, require explicit HTTP headers configured in deployment configuration (like `vercel.json` or `next.config.js`) to provide defense in depth against client-side attacks. The docs/SECURITY.md contained the recommendations but they weren't implemented.
**Prevention:** Include standard security headers in the deployment configuration template (`vercel.json`) from the start, specifically implementing a robust `Content-Security-Policy` that restricts `script-src` and `connect-src` to known good origins.
