...
## 2025-02-27 - Context-Aware URL Sanitization for Images
**Vulnerability:** XSS vulnerability through unsanitized URL inputs bound directly to `<img src={...} />` attributes, where a generic URL sanitizer might inappropriately strip or allow `data:` payloads.
**Learning:** Generic URL sanitizers (e.g., `getSafeUrl`) designed for `href` attributes should not blindly allow `data:` protocols, as this can introduce `data:text/html` XSS vectors in link tags. However, `data:` protocols are often legitimately required for inline images in `<img src={...}>` tags. Re-using generic sanitizers across different HTML context boundaries can lead to either broken functionality (stripping safe image data) or security regressions.
**Prevention:** Implement context-aware sanitization functions. For images, use a dedicated `getSafeImageUrl` function that explicitly allows `data:` protocols but strictly validates that the MIME type begins with `image/` (e.g., `data:image/`), mitigating non-image payload injection while preserving legitimate functionality.
