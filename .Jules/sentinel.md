...
**Vulnerability:** The application was missing crucial security headers like Content-Security-Policy (CSP), Strict-Transport-Security (HSTS), X-Frame-Options, X-Content-Type-Options, and Referrer-Policy in the production configuration (`vercel.json`). This exposed the app to XSS, Clickjacking, MIME-sniffing, and MITM attacks.
**Learning:** Modern web deployments require explicit security headers configured at the hosting platform level (like Vercel or Netlify) to provide defense-in-depth, even if the application code itself is robust against common vulnerabilities.
**Prevention:** Always define a strict CSP and standard security headers in the deployment configuration file (`vercel.json`, `netlify.toml`, etc.) before launching to production. Ensure `connect-src` specifically allowlists backend APIs (e.g., Supabase endpoints).
