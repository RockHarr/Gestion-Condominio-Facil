...
## 2026-03-02 - [Predictable Order and Transaction IDs]
**Vulnerability:** The application was using `Math.random()` to generate Order IDs and Transaction IDs in the `PaymentConfirmScreen` and `PaymentReceiptScreen`. `Math.random()` is not cryptographically secure and produces predictable outputs. This could potentially allow attackers to guess or brute-force identifiers, leading to information disclosure, ID collision issues, or bypassing sequential checks. Furthermore, calling random generation directly in the component render body caused the IDs to regenerate on every re-render.
**Learning:** Any identifier exposed to the user or used for tracking sensitive workflows (like payments or transactions) must be generated using cryptographically secure methods, even if it's "just" for UI display. Additionally, in React, generated identifiers should be stateful to prevent unintended mutations during the component lifecycle.
**Prevention:** Use Web Crypto API (`crypto.getRandomValues()` or `crypto.randomUUID()`) to generate secure identifiers. In React components, wrap the generation logic inside a `useState` lazy initializer function (e.g., `useState(() => crypto.randomUUID())`) to ensure the value remains constant across re-renders.
