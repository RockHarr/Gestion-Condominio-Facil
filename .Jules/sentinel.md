...
## 2024-05-23 - Hardcoded Credentials in Tests and Scripts
**Vulnerability:** Found multiple instances of hardcoded Supabase URLs, Anon Keys, and user/admin passwords directly committed in test files (`tests/e2e/*.spec.ts`, `tests/repro_rpc.*`) and utility scripts (`scripts/*`).
**Learning:** Hardcoding credentials, even for testing environments, in version-controlled files is a severe security risk. It can lead to unauthorized access to test or even production databases if those environments share configurations, and pollutes the codebase with secrets.
**Prevention:** Always use environment variables (e.g., `process.env.VITE_SUPABASE_URL`, `process.env.TEST_RESIDENT_PASSWORD`) to inject sensitive data into tests and scripts. Provide generic fallback values (like `http://127.0.0.1:54321` or `dummy_key`) if variables are absent, ensuring tests can still run locally or in CI without requiring real secrets in the code.
