...
## 2025-02-14 - Unsanitized Image Source URLs
**Vulnerability:** Dynamically rendered user-supplied URLs (like `ticket.foto` and `amenity.photoUrl`) were bound directly to `<img src={...}>` without sanitization. While XSS in `img src` doesn't execute JavaScript in most modern browsers directly, an unsanitized URL could still trigger SSRF, tracking pixels, or open up vulnerabilities if the URL is ever reused in an `href` context. More importantly, it violated the explicit internal security guideline that all such dynamically rendered URLs must use `getSafeUrl`.
**Learning:** React escapes text nodes, but attributes like `src` or `href` taking dynamic URL variables remain vectors for malicious schemas or payloads if the schema (`javascript:`, `data:`, etc.) isn't validated.
**Prevention:** Always use `getSafeUrl()` from `lib/sanitize` to strip control characters and restrict protocols to an allowlist (`http:`, `https:`, etc.) before binding any user-controlled URL string to an HTML attribute like `src` or `href`.
