...
## 2026-03-30 - [Hardcoded Passwords in E2E Tests]
**Vulnerability:** Several End-to-End (E2E) test files in the `tests/` directory contained hardcoded email addresses and plain-text passwords (e.g., `contacto@rockcode.cl`, `180381`, `rockwell.harrison@gmail.com`, `270386`). Hardcoding secrets, even in test files, poses a CRITICAL security risk as it exposes actual or potential production credentials to anyone with read access to the repository.
**Learning:** Hardcoded credentials are a severe violation of secure coding practices. Even if the credentials are only meant for a staging or testing environment, they should never be checked into version control. Doing so risks credential leakage and unauthorized access.
**Prevention:** Always use environment variables for sensitive configuration data. In test environments, create a dedicated configuration file (like `test-config.ts`) that reads from `process.env` and provides safe, dummy fallback values (never the actual secrets) if the environment variables are not set. Test files should then import and use these configuration variables.
