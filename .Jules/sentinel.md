...
## 2026-03-02 - [XSS Defense-in-Depth for Image URLs]
**Vulnerability:** User-provided image URLs (e.g., `ticket.foto`, `amenity.photoUrl`) were rendered directly into `<img>` `src` attributes without sanitization. While modern browsers block `javascript:` execution in `<img>` tags, defense-in-depth requires strictly controlling allowable protocols to prevent unforeseen exploits or the loading of unauthorized resources.
**Learning:** URL sanitizers designed for `href` attributes (like `getSafeUrl`) are often insufficient or incorrect for image sources. Specifically, they may block legitimate `data:image/*` URIs used for local file previews, while potentially allowing risky protocols if reused blindly.
**Prevention:** Implement context-aware URL sanitization (e.g., a distinct `getSafeImageUrl` function) that explicitly validates the protocol allowlist specifically for image contexts (e.g., allowing `http:`, `https:`, `blob:`, and enforcing `image/*` MIME types for `data:` URIs). Always apply this sanitizer to all dynamic `src` attributes.
