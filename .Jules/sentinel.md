...
## 2025-02-15 - [Added Security Headers to vercel.json]
**Vulnerability:** The application was missing critical security headers (Content Security Policy, X-Content-Type-Options, X-Frame-Options, X-XSS-Protection, Referrer-Policy) in the production Vercel deployment. This made the application vulnerable to XSS attacks, clickjacking, MIME-sniffing, and other web vulnerabilities.
**Learning:** The application relies on external CDNs (like `https://aistudiocdn.com` and `https://cdn.tailwindcss.com`) and connects to Supabase instances (`https://*.supabase.co`, `wss://*.supabase.co`). Therefore, configuring a Strict Content Security Policy (CSP) required accurately identifying and whitelisting all legitimate external domains to avoid breaking the application, including development/CI dummies (`http://127.0.0.1:54321`) to ensure tests wouldn't fail on a production build.
**Prevention:** In Vite/Vercel projects, security headers should be configured in `vercel.json` since they operate on the edge. Always ensure all external script, style, image, and connect endpoints are explicitly added to the CSP directives to maintain both security and functionality.
