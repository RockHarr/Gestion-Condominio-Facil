...
## 2025-04-19 - [Insecure Randomness for Transaction and Order IDs]
**Vulnerability:** The application was using `Math.random()` to generate critical identifiers like Purchase Order (`orderId`) and Transaction ID (`transactionId`) in the `PaymentsScreen`. `Math.random()` is not cryptographically secure and can generate predictable patterns or collisions, which could theoretically allow an attacker to guess transaction IDs. Furthermore, the random generation was placed directly in the React render flow, causing these IDs to change continuously during component re-renders.
**Learning:** `Math.random()` must never be used for security-sensitive or unique identifying values like transaction IDs, session tokens, or passwords.
**Prevention:** Use the cryptographically secure `crypto.randomUUID()` when generating random identifiers on the frontend. Additionally, in React, wrap random ID generation in a lazy `useState` initialization (`useState(() => crypto.randomUUID())`) to ensure the value remains consistent across component re-renders.
