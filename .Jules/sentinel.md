...
## 2026-03-01 - [Predictable Random Number Generation for Display IDs]
**Vulnerability:** The application was using `Math.random()` to generate mock order and transaction IDs on the frontend (`PaymentsScreen.tsx`). While these were mock values, using cryptographically weak pseudo-random number generators (PRNGs) for identifiers can lead to predictability and collision issues. If this pattern were adopted for generating real identifiers, it could be exploited to guess transaction IDs.
**Learning:** Any randomly generated identifier (like a transaction ID or session token) should use a cryptographically secure random number generator (CSPRNG), such as the Web Crypto API, to ensure uniqueness and unpredictability. Additionally, in React, generated values meant to persist across renders should be wrapped in a lazy `useState` initializer to prevent unintended regeneration.
**Prevention:** Use `crypto.randomUUID()` to generate secure random identifiers on the frontend. Ensure these generations are tied to a stable component state (e.g., `const [id] = useState(() => crypto.randomUUID())`) rather than being evaluated on every render.
