...
## 2026-04-18 - [Insecure Random Number Generation for Identifiers]
**Vulnerability:** The application used `Math.random()` to generate transaction and order IDs in `components/PaymentsScreen.tsx` (e.g., `Math.floor(Math.random() * 1000000)` and `Math.random().toString(36).substring(...)`). `Math.random()` is not cryptographically secure, leading to predictable IDs that could be subject to enumeration or collisions.
**Learning:** For any identifier that needs to be unique and secure (like transaction IDs, order numbers, tokens), cryptographically secure random number generators (CSRNG) must be used instead of standard pseudorandom functions. Additionally, when generating random values during rendering in React, they should be wrapped in a lazy `useState` initializer to prevent unintended regeneration upon re-renders.
**Prevention:** Use the Web Crypto API (`crypto.randomUUID()` or `crypto.getRandomValues()`) for generating secure random IDs on the frontend. Wrap generation logic in `useState(() => ...)` to maintain identifier stability across component re-renders.
