...
## 2026-02-16 - [Privilege Escalation via Profile Updates Resolved]
**Vulnerability:** Confirmed the "Privilege Escalation via Profile Updates" vulnerability where users could self-promote to admin. The previous identification (2026-01-24) was correct but the fix was not permanently applied to the schema.
**Learning:** Migration files alone are insufficient if `schema.sql` (the source of truth for new deploys) is not updated. Security fixes must be applied to both migration history and the base schema definition.
**Prevention:** Updated `supabase/schema.sql` to include `prevent_role_escalation` trigger by default.
