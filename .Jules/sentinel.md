...
## 2026-03-02 - [Broken Access Control via Truthy Check on userId]
**Vulnerability:** The data service methods for retrieving debts and payments (`getCommonExpenseDebts`, `getParkingDebts`, `getPaymentHistory`) used a truthiness check (`if (userId)`) to conditionally apply user-specific filtering to database queries. This meant that if a user ID of `0` (or `""`) was passed, the filter was bypassed, potentially leaking the financial records of all users in the system instead of restricting it to that specific user.
**Learning:** Using simple truthy checks (`if (value)`) for authorization logic or data filtering is dangerous when valid falsy values (like `0` or `""`) can be passed. This can lead to Broken Access Control and information disclosure.
**Prevention:** Always use explicit nullish checks (`if (value !== undefined && value !== null)` or `if (value ?? false)`) when determining whether an optional parameter has been provided for security-critical filters.
