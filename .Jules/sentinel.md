...
## 2026-03-01 - [XSS Vulnerability in Image Attributes]
**Vulnerability:** The application was directly rendering user-provided URLs in `img` `src` attributes without sanitization. This allowed for potential Cross-Site Scripting (XSS) if an attacker could input a malicious payload (e.g., `javascript:alert(1)` or a non-image `data:text/html` URI) into the URL.
**Learning:** Any user-supplied data used in attributes like `href` or `src` must be treated as untrusted and sanitized before rendering to follow the principle of defense-in-depth. `data:` URIs are especially dangerous because they can encapsulate executable content if the MIME type is not restricted to images.
**Prevention:** Use a dedicated sanitization function like `getSafeImageUrl` to validate the URL's protocol against an allowlist (e.g., `http:`, `https:`, `blob:`, `data:`) before rendering it in the UI. For `data:` URIs, explicitly validate that the MIME type begins with `image/`.
