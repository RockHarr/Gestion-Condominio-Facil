...
## 2026-06-03 - [Insecure Data Access due to Truthy Evaluation]
**Vulnerability:** The data-fetching functions in `services/data.ts` (e.g., `getCommonExpenseDebts`, `getParkingDebts`, `getPaymentHistory`) were filtering by `userId` using a simple truthy evaluation: `if (userId) query = query.eq('user_id', userId);`. This meant if a user's ID resolved to `0`, the check failed, the filter was not applied, and the query inadvertently returned all records for all users (a form of Insecure Direct Object Reference).
**Learning:** Checking for the presence of optional, primitive IDs (like `0` or `''`) must always use strict type-checking instead of truthiness. Relying on truthiness can disable critical access control filters when valid "falsy" identifiers are provided.
**Prevention:** Whenever filtering by an optional parameter that could be `0` or an empty string, always use strict checks: `if (userId !== undefined && userId !== null)` to ensure the security policy is not bypassed.
