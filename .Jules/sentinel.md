...
**Vulnerability:** The application was directly rendering user-supplied data in \`<img>\` \`src\` attributes (e.g., \`ticket.foto\`, \`amenity.photoUrl\`, etc.) without strict sanitization. While React auto-escapes most inputs, it doesn't prevent malicious protocols (like \`javascript:\` if somehow executed by older browsers or specific contexts, though modern browsers generally block this in \`<img>\`) or other unexpected URL structures.
**Learning:** Any user-supplied data used in attributes like \`href\` or \`src\` must be treated as untrusted and sanitized before rendering, even if it comes from a supposedly secure backend or database, to follow the principle of defense-in-depth. Using a generic URL sanitizer might inadvertently break valid image data URLs (\`data:\`).
**Prevention:** Use a dedicated sanitization function like \`getSafeImageUrl\` to validate the URL's protocol against an allowlist (e.g., \`http:\`, \`https:\`, \`blob:\`, \`data:\`) before rendering it in \`<img>\` tags.
