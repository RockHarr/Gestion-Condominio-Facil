...
## 2025-02-27 - [MEDIUM] Fix insecure random number generation using crypto.randomUUID()
**Vulnerability:** The application was using `Math.random()` to generate Order IDs and Transaction IDs on the frontend (`PaymentConfirmScreen` and `PaymentReceiptScreen`). `Math.random()` is not cryptographically secure, leading to predictability and potential collision vulnerabilities for identifier generation.
**Learning:** In React components, when generating secure random identifiers for display purposes using `crypto.randomUUID()`, simply replacing `Math.random()` directly in the render body causes the identifier to regenerate on every component re-render. This unintended regeneration breaks UI consistency and could interrupt payment flows.
**Prevention:** Always wrap the cryptographically secure generation logic in a `useState` lazy initializer (e.g., `const [orderId] = useState(() => crypto.randomUUID().split('-')[0].toUpperCase());`). This ensures the secure identifier is generated only once per component lifecycle while remaining secure.
