...
## 2025-06-27 - Action Buttons Keyboard Accessibility
**Learning:** Action buttons in lists or cards hidden visually using `opacity-0 group-hover:opacity-100` for cleaner UI are completely inaccessible to keyboard users because they cannot see which action they are focused on when tabbing.
**Action:** Whenever using `opacity-0 group-hover:opacity-100` to hide elements until hover, always pair it with `group-focus-within:opacity-100` or `focus-within:opacity-100` to ensure keyboard accessibility.
