...
## 2025-06-23 - Keyboard Navigation for Hover Actions
**Learning:** Action buttons hidden behind `opacity-0 group-hover:opacity-100` become completely inaccessible to keyboard-only users navigating with the Tab key.
**Action:** Always pair `group-hover:opacity-100` with `group-focus-within:opacity-100` (or `focus-within:opacity-100`) to ensure interactive elements become visible when focused. Also, always provide `aria-label`s for icon-only buttons to aid screen readers.
