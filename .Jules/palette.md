...
## 2025-08-31 - Amenity Manager Empty State & Accessibility
**Learning:** Found multiple icon-only buttons in the Amenities Manager without `aria-label` or `title` attributes, making screen reader navigation impossible. There is also an empty state that uses a custom container rather than the reusable `EmptyState` component. Replaced the custom empty state for consistency across the application and updated icon-only buttons for full accessibility compliance.
**Action:** Always check the `AmenitiesManager` for accessibility and make sure to use `EmptyState` when listing elements.
