...
## 2025-07-04 - Keyboard accessibility for hidden hover actions
**Learning:** In this application, elements hidden with `opacity-0 group-hover:opacity-100` break keyboard accessibility because they can be focused but remain invisible. The `group-focus-within:opacity-100` class is required to ensure these interactive elements become visible when focused via the Tab key.
**Action:** Always add `group-focus-within:opacity-100` or `focus-within:opacity-100` when using hover-based opacity toggles on containers with interactive elements.
