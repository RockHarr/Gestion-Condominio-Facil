...
## 2024-04-20 - Adding ARIA labels to icon-only modal close buttons
**Learning:** Found an accessibility issue pattern where modal close buttons (using an `X` or similar icon) lack accessible names, making them difficult for screen reader users to identify their purpose.
**Action:** Always ensure that icon-only interactive elements, such as `<button>` elements containing only an `<Icons />` component, are provided with a descriptive `aria-label` attribute (e.g., `aria-label="Cerrar modal"`).
