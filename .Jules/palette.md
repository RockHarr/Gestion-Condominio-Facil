...
## 2025-05-25 - Hover States and Keyboard Focus Visibility
**Learning:** Action buttons in list items that use `opacity-0 group-hover:opacity-100` are invisible to keyboard users who navigate via `Tab`. The buttons receive focus, but remain visually hidden, confusing the user.
**Action:** Always pair `group-hover:opacity-100` with `focus-within:opacity-100` on the container so elements reveal themselves when any child receives keyboard focus. Also ensure icon-only buttons define explicit `type="button"` and `aria-label` attributes.
