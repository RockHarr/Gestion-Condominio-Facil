...
## 2025-05-27 - Action Buttons Focus Visibility
**Learning:** Container actions hidden with `opacity-0 group-hover:opacity-100` are completely inaccessible to keyboard users as they never become visible on `tab` focus.
**Action:** When implementing hover-only actions, always pair them with `group-focus-within:opacity-100` on the container and `focus:outline-none focus-visible:ring-2` on the elements to maintain strict keyboard accessibility.
