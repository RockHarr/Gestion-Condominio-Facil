...
## 2024-08-08 - Accessible Hover Actions
**Learning:** Hidden action buttons revealed only on hover (`group-hover:opacity-100`) are inaccessible to keyboard navigation because they remain invisible (and sometimes unfocusable or visually confusing) when a user tabs to them.
**Action:** Always pair `group-hover:opacity-100` with `group-focus-within:opacity-100` on the container to ensure the actions become visible when any element inside the container receives focus.
