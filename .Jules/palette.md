...
## 2025-06-26 - Missing Accessible Labels on Icon-only Modals
**Learning:** Many custom modal components and specific UI states (like photo removal in TicketsScreen) rely on an icon-only button (`<Icons name="xmark" />`) without an explicit `aria-label`. This makes the action invisible to screen readers.
**Action:** Always add an explicit, translated `aria-label` (e.g., `aria-label="Cerrar modal"` or `aria-label="Quitar foto"`) to any icon-only button (`<button><Icons ... /></button>`).
