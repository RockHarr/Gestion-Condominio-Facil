...
## 2024-07-19 - Ensure Keyboard Access for Hover-Revealed Actions
**Learning:** When using `opacity-0 group-hover:opacity-100` to hide actions until hovered, keyboard users miss out on seeing these actions when tabbing through them. They gain focus but remain invisible.
**Action:** Always pair `group-hover:opacity-100` with `group-focus-within:opacity-100` on the container, and add `focus-visible:ring-2` to the interactive elements themselves so they show a ring when focused via keyboard.
