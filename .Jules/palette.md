...
## 2025-05-24 - Icon-only buttons Accessibility

**Learning:** Found multiple instances of icon-only buttons (like "close modal" buttons or "remove photo") missing `aria-label` attributes across different components (e.g., `AdminCreateReservationModal`, `ReservationRequestModal`, `TicketsScreen`). This makes the application less accessible to screen reader users who cannot infer the button's action from an icon alone.
**Action:** Always add descriptive `aria-label`s to `button` elements when they contain only an `Icon` component. Ensure new modal close buttons and other action buttons have appropriate labels.
