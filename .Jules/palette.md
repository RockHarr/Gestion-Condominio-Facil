...
## 2024-07-22 - Keyboard navigation for hover-revealed action buttons
**Learning:** Action buttons that are only revealed on `group-hover` become completely invisible to keyboard-only users navigating via tab stops, failing accessibility standards.
**Action:** When implementing hover-revealed actions using Tailwind's `group-hover:opacity-100`, always pair it with `group-focus-within:opacity-100` on the container, and add `focus-visible:ring-2` styling alongside translated `aria-label` attributes to the internal buttons to ensure they are discoverable and usable by keyboard navigation and screen readers.
