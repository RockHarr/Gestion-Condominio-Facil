...
## 2025-05-25 - Interactive Hover State Accessibility
**Learning:** Container actions hidden by `opacity-0 group-hover:opacity-100` are invisible to keyboard-only users who tab to the underlying buttons, causing a confusing invisible focus trap.
**Action:** Always pair `group-hover:opacity-100` with `group-focus-within:opacity-100` (or `focus-within:opacity-100`) on the hidden action container so it reveals itself when any of its child buttons receive keyboard focus.
