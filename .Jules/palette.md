...
## 2025-08-20 - Keyboard Accessibility for Hover Actions
**Learning:** Actions hidden via Tailwind's `opacity-0 group-hover:opacity-100` are completely inaccessible to keyboard users because they lack a focus state to make them visible and focusable.
**Action:** When implementing hover-revealed actions, always pair `group-hover:opacity-100` with `group-focus-within:opacity-100` on the container, and ensure interactive elements have `focus-visible:ring-2` and an `aria-label` for screen readers.
