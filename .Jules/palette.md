...
## 2025-05-24 - Accessibility for Hover-Revealed Elements
**Learning:** Hover-hidden element containers (e.g., using `opacity-0 group-hover:opacity-100`) completely hide interactive elements from keyboard-only users who navigate via `Tab`.
**Action:** Always append `focus-within:opacity-100` (or `group-focus-within:opacity-100`) to containers that reveal children on hover. Additionally, ensure all icon-only buttons receive an explicit `aria-label` and `focus-visible:ring-2` for a clear focus state.
