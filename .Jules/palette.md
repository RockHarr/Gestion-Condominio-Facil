...
## 2024-05-20 - Ensure Action Buttons in Hover Containers are Focusable
**Learning:** In the `AmenitiesManager.tsx`, `ReservationTypesManager.tsx`, and `AdminDashboard.tsx`, interactive edit/delete buttons inside cards were hidden visually using `opacity-0 group-hover:opacity-100`. Because they didn't have `focus-within:opacity-100` on the container, keyboard users could tab to these buttons but they would remain completely invisible, making it impossible to know what they were about to activate or delete.
**Action:** When hiding elements behind a hover state using `opacity-0 group-hover:opacity-100`, always pair it with `group-focus-within:opacity-100` or `focus-within:opacity-100` on the container. This guarantees that when a keyboard user tabs to an interactive element inside the container, the entire action group becomes visible, maintaining strict keyboard accessibility without breaking the visual hover pattern.
