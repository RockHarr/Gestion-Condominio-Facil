...
## 2025-06-06 - Icon-Only Button Accessibility and Focus States
**Learning:** Icon-only interactive elements (like modal close buttons or image removal buttons) frequently lack accessible names (`aria-label`) and explicit keyboard focus states (`focus-visible:ring-*`), making them invisible to screen readers and difficult to navigate via keyboard. Also, `<button>`s without `type="button"` inside or near forms risk triggering accidental submissions.
**Action:** Always provide an `aria-label` indicating the action (e.g., "Cerrar modal", "Eliminar foto") for buttons containing only an `<Icons />`. Enforce `type="button"` consistently, and apply `focus:outline-none focus-visible:ring-2 focus-visible:ring-*` to guarantee keyboard accessibility without compromising mouse/touch click aesthetics.
