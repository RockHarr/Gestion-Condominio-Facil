...
**Learning:** Icon-only buttons without `aria-label` or `title` properties violate accessibility standards and are common in modals and admin dashboards.
**Action:** Always review icon-only buttons (`<button><Icons/></button>`) across modals and manager views to ensure they include an `aria-label` that explains the action for screen reader users.
