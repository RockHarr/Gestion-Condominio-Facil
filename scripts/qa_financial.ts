...
const supabaseKey =
  ...
  'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InRxc2hvZGRpaXNmZ2ZqcWxrbnR2Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NjY2ODQzMTAsImV4cCI6MjA4MjI2MDMxMH0.eiD6ZgiBU3Wsj9NfJoDtX3J9wHHxOVCINLoeULZJEYc';
...
async function testFinancials() {
  ...
  const { data: payments, error: payError } = await supabase.from('payments').select('monto');
  ...
  if (!residents || !settings) {
    ...
    residents.slice(0, 3).forEach((r) => {
      ...
      console.log(
        ` - User ${r.id.slice(0, 8)}... | Alicuota: ${alicuota}% | Calc Debt: $${debt} | Parking: ${r.has_parking ? '$' + parkingCost : '$0'}`,
      );
    });
  }
}
