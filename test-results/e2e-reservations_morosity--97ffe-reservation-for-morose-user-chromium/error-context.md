...
```ts
  ...
  11  | const RESIDENT_PASSWORD = process.env.TEST_RESIDENT_PASSWORD || 'dummy_password'; // Assuming this is the password from previous context
  12  |
...
> 26  |             throw new Error('Failed to login as resident for setup: ' + residentError?.message);
  ...
  50  |             password: process.env.TEST_ADMIN_PASSWORD || 'dummy_admin_password'
  51  |         });
  52  |
  53  |         if (adminError) {
