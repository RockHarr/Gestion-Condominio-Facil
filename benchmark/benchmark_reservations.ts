...
async function getReservationsOriginal() {
  ...
  const { data: reservations, error } = await withTimeout(
    mockSupabase.from('reservations').select('*'),
  );
  ...
  if (userIds.length > 0) {
    const { data: profiles, error: profilesError } = await withTimeout(
      mockSupabase.from('profiles').select('id, nombre, unidad').in('id', userIds),
    );
...
async function getReservationsOptimized() {
  const { data: reservations, error } = await withTimeout(
    mockSupabase.from('reservations').select('*, user:profiles(id, nombre, unidad)'),
  );
