...
export const dataService = {
  ...
  async createTicket(ticket: Omit<Ticket, 'id' | 'fecha' | 'user' | 'estado'>, userId: string) {
    ...
    if (ticket.titulo.length > 100) throw new Error('El título no puede exceder los 100 caracteres');
    if (ticket.descripcion.length > 2000)
      throw new Error('La descripción no puede exceder los 2000 caracteres');
    if (ticket.foto && ticket.foto.length > 7 * 1024 * 1024 * 1.37)
      throw new Error('La foto excede el tamaño máximo permitido (7MB)');
  ...
  async updateTicketStatus(id: number, estado: TicketStatus) {
    const { error } = await withTimeout(supabase.from('tickets').update({ estado }).eq('id', id));
    if (error) throw error;
  },
  ...
  async createNotice(notice: Omit<Notice, 'id' | 'fecha' | 'leido' | 'status'>) {
    // 🛡️ Sentinel: Input Validation
    if (notice.titulo.length > 100) throw new Error('El título no puede exceder los 100 caracteres');
  ...
  async getSettings() {
    const { data, error } = await withTimeout(
      supabase.from('community_settings').select('*').single(),
    );
  ...
  async getMyVote(pollId: number) {
    const { data, error } = await withTimeout(
      supabase.from('poll_responses').select('option_id').eq('poll_id', pollId).maybeSingle(),
    );
  ...
  async getReservations() {
    ...
    const { data: reservations, error } = await withTimeout(
    ...
    let profiles: Pick<ProfileRow, 'id' | 'nombre' | 'unidad'>[] = [];
    try {
      const typedReservations = reservations as ReservationRow[];
      const userIds = [
        ...new Set(
          typedReservations.map((r) => r.user_id).filter((id): id is string => Boolean(id)),
        ),
      ];
      if (userIds.length > 0) {
        const { data } = await supabase
          ...
          .in('id', userIds);
        profiles = (data || []) as Pick<ProfileRow, 'id' | 'nombre' | 'unidad'>[];
      }
    } catch (e) {
    ...
    // 3. Map data
    return (reservations as ReservationRow[]).map((r) => ({
      id: r.id,
      ...
      formData: r.form_data as FormData | undefined,
      user: r.user_id ? profileMap.get(r.user_id) : undefined,
    })) as Reservation[];
  },
  ...
  async getAmenities() {
    ...
    return (data as AmenityRow[]).map((a) => ({
      id: a.id,
      name: a.name,
  ...
  async createReservation(reservation: Pick<Reservation, 'amenityId' | 'startAt' | 'endAt'>) {
    ...
    // 2. Call RPC
    const { data, error } = await withTimeout(
      supabase.rpc('request_reservation', {
  ...
  async createReservationAsAdmin(
    amenityId: string,
    userId: string,
    startAt: string,
    endAt: string,
    ...
    const { data, error } = await withTimeout(
      supabase.rpc('create_reservation_as_admin', {
  ...
  async cancelReservation(id: number) {
    const { error } = await withTimeout(
      supabase.rpc('cancel_reservation', { p_reservation_id: id }),
    );
    if (error) throw error;
  },
  ...
  async rejectReservation(id: number) {
    const { error } = await withTimeout(
      supabase.from('reservations').update({ status: 'REJECTED' }).eq('id', id),
    );
    if (error) throw error;
  },
  ...
  async getChargesByReference(
    referenceType: 'RESERVATION' | 'INCIDENT' | 'MONTH',
    referenceId: number,
  ) {
    const { data, error } = await withTimeout(
      supabase
  ...
  async confirmChargePayment(chargeId: string, method: string, note?: string) {
    const { data, error } = await withTimeout(
      supabase.rpc('confirm_charge_payment', {
  ...
  async createSystemReservation(amenityId: number, startAt: string, endAt: string, reason: string) {
    const { data, error } = await withTimeout(
      supabase.rpc('create_system_reservation', {
        p_amenity_id: amenityId,
        p_start_at: startAt,
        p_end_at: endAt,
        p_reason: reason,
  ...
  async getUsers() {
    const { data, error } = await withTimeout(
      supabase.from('profiles').select('*').order('unidad', { ascending: true }),
    );
  ...
  async deleteUser(id: string | number) {
    const { error } = await withTimeout(supabase.from('profiles').delete().eq('id', id));
  ...
  async approveNotice(id: number) {
    const { error } = await withTimeout(
      supabase.from('notices').update({ status: 'Publicado' }).eq('id', id),
    );
  ...
  async addExpense(expense: Omit<Expense, 'id' | 'status' | 'fecha' | 'motivoRechazo'>) {
    ...
    if (expense.monto <= 0) throw new Error('El monto debe ser mayor a 0');
  ...
  async approveExpense(id: number) {
    const { error } = await withTimeout(
      supabase.from('expenses').update({ status: 'Aprobado' }).eq('id', id),
    );
  ...
  async closeMonthAndGenerateStatement() {
    ...
    const { data: expenses, error: expError } = await withTimeout(
      supabase.from('expenses').select('*').eq('status', 'Aprobado'),
    );
    ...
    const { data: payments, error: payError } = await withTimeout(
      supabase.from('payments').select('monto').eq('periodo', currentMonthStr),
    );
    ...
    const { data: lastStatement } = await withTimeout(
      supabase
    ...
    const { data: residents, error: resError } = await withTimeout(
      supabase.from('profiles').select('id, alicuota, has_parking').eq('role', 'resident'),
    );
    ...
    if (residents) {
      ...
      const { data: settings } = await withTimeout(
        supabase.from('community_settings').select('parking_cost_amount').single(),
      );
      ...
      if (debtsToInsert.length > 0) {
        const { error: debtError } = await withTimeout(
          supabase.from('common_expense_debts').insert(debtsToInsert),
        );
        if (debtError) console.error('Error inserting common expense debts', debtError);
      }

      if (parkingDebtsToInsert.length > 0) {
        const { error: parkError } = await withTimeout(
          supabase.from('parking_debts').insert(parkingDebtsToInsert),
        );
    ...
    const { data: newStatement, error: stmtError } = await withTimeout(
      supabase
        .from('financial_statements')
        .insert({
          mes: mesNombre,
  ...
  async updateCommunitySettings(settings: CommunitySettings) {
    const { error } = await withTimeout(
      supabase.from('community_settings').update(settings).eq('id', 1),
    );
