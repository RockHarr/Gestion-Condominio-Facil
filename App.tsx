...
import { TicketStatus, NoticeStatus, getErrorMessage } from './types';
...
function App() {
    ...
    async (userArg?: User) => {
      ...
      try {
        ...
        if (targetUser?.role === 'resident') {
          ...
          // Admin fetching
          const [allTickets, allPayments, allCommonDebts, allParkingDebts, allUsers] =
            await Promise.all([
  ...
  useEffect(() => {
    ...
    // Safety timeout - Increased for CI/Demo environments
  ...
  const handleNavigate = (newPage: Page, params: PageParams | null = null) => {
    window.scrollTo(0, 0);
    setPage(newPage);
  ...
  const unreadNoticesCount = useMemo(
    () => notices.filter((n) => !n.leido && n.status === NoticeStatus.PUBLICADO).length,
    [notices],
  );
  ...
  const addNotice = async (noticeData: Omit<Notice, 'id' | 'fecha' | 'leido' | 'status'>) => {
    try {
      ...
      showToast(`Error al crear aviso: ${getErrorMessage(error)}`, 'error');
    }
  };
  ...
  const approveNotice = async (id: number) => {
    try {
      ...
      showToast(`Error al publicar aviso: ${getErrorMessage(error)}`, 'error');
    }
  };
  ...
  const handleUpdateUser = async (
    ...
    try {
      ...
      showToast(`Error al aprobar gasto: ${getErrorMessage(error)}`, 'error');
    }
  };
  ...
  const handleApproveExpense = async (id: number) => {
    try {
      ...
      console.error('App: Error creating expense', error);
  ...
  const handleRejectExpense = async (id: number, motivo: string) => {
    try {
      ...
      showToast(`Error al actualizar unidad: ${getErrorMessage(error)}`, 'error');
    }
  };
  ...
  const handleCloseMonth = async () => {
    if (
      window.confirm(
        '¿Estás seguro de cerrar el mes? Esta acción generará un nuevo informe financiero con los gastos aprobados y no se puede deshacer.',
      )
    ) {
      try {
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center p-4 text-center space-y-4">
        ...
        <p className="text-gray-500 dark:text-gray-400 max-w-md">
          No se pudo obtener la configuración del condominio. Verifique su conexión o contacte al
          administrador.
        </p>
        {/* Debug info */}
